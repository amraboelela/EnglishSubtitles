...
class SpeechRecognitionServiceTests {
    ...
    @Test func testTranslationWithTurkishAudioFile() async throws {
        // This test uses the actual audio file for translation testing
        if !(await Self.service.isReady) {
            await Self.service.loadModel()
        }
        let isReady = await TestHelpers.waitForWhisperKit(Self.service)
        ...
        let englishText = try await Self.service.processAudioFile(at: audioURL, task: .translate)
    ...
    @Test func testProgressCallback() async throws {
        ...
        // Test the progress callback mechanism with SpeechRecognitionService
